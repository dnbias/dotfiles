...
(use-package! org-roam
  ...
  (org-roam-capture-templates
   '(("d" "default" plain
      ...
      :unnarrowed t))
    (("b" "book note" plain
      (file (concat templates-directory "/BookNote.org"))
      :if-new (file+head "${slug}.org" "#+title: ${title}\n#+date: %U\n")
      :unnarrowed t))
    (("a" "author" plain
      (file (concat templates-directory "/Author.org"))
      :if-new (file+head "${slug}.org" "#+title: ${title}\n#+date: %U\n")
      :unnarrowed t))
    (("v" "video note" plain
      (file (concat templates-directory "/VideoNote.org"))
      :if-new (file+head "${slug}.org" "#+title: ${title}\n#+date: %U\n")
      :unnarrowed t))
    (("c" "podcast note" plain
      (file (concat templates-directory "/PodcastNote.org"))
      :if-new (file+head "${slug}.org" "#+title: ${title}\n#+date: %U\n")
      :unnarrowed t))
    (("n" "article note" plain
      (file (concat templates-directory "/ArticleNote.org"))
      :if-new (file+head "${slug}.org" "#+title: ${title}\n#+date: %U\n")
      :unnarrowed t))
    (("p" "project" plain
      (file (concat templates-directory "/Project.org"))
      :if-new (file+head "${slug}.org" "#+title: ${title}\n#+date: %U\n")
      :unnarrowed t))
     )
  :config
